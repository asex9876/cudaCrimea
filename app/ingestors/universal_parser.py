...
[
  {
    ...
    "image_url": "URL изображения события (из списка IMG_URLS ниже, выбери наиболее подходящее)"
  }
]
```
...
def extract_image_urls(html: str, base_url: str) -> list[str]:
    ...
    for img in soup.find_all('img'):
        ...
        if src:
            ...
            if not any(x in absolute_url.lower() for x in ['icon', 'logo', 'avatar', 'pixel', '1x1', 'spacer']):
    ...
    # Limit to first 20 images to avoid token overflow
...
async def parse_with_ai(html_content: str, source_url: str) -> list[dict[str, Any]]:
    ...
    image_urls = extract_image_urls(html_content, source_url)
    ...
    img_urls_text = ""
    if image_urls:
        img_urls_text = "\n\nIMG_URLS (доступные изображения):\n" + "\n".join(f"- {url}" for url in image_urls)

    user_message = f"URL: {source_url}\n\nHTML содержимое:\n{cleaned}{img_urls_text}"
        ...
        for event_data in events_data:
            try:
                ...
                extracted_data = {
                    ...
                    "image_url": event_data.get("image_url"),  # AI-extracted image URL
                }
